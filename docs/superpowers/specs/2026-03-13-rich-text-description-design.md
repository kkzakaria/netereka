...
**Rationale:**
- Storefront (high traffic): server-side JSON→HTML conversion in RSC is negligible (few ms CPU on Cloudflare Workers).
...
- Lexical JSON → parse and convert to HTML server-side via `@lexical/html`
- Legacy raw HTML → render directly with `dangerouslySetInnerHTML`
- Plain text → wrap in `<p>` tags (existing behaviour)
...
Lexical is client-only. The component is imported with `dynamic(() => import(...), { ssr: false })` in `product-form-sections.tsx` to prevent SSR errors on Cloudflare Workers.
...
`DescriptionContent` in `components/storefront/product-details.tsx` becomes a server-side utility function.
...
function descriptionToHtml(raw: string): string {
  ...
  if (trimmed.startsWith("{") && trimmed.includes('"root"')) {
    return convertLexicalJsonToHtml(trimmed); // server-side via @lexical/html
  }

  // Legacy raw HTML
  if (trimmed.startsWith("<")) {
    return sanitizeHtml(trimmed); // allowlist-based sanitisation
  }
...
- **Lexical JSON output** is trusted (generated by the serialiser, not direct user input) — no sanitisation needed.
- **Legacy HTML** (already in the database) is sanitised using a simple tag/attribute allowlist to prevent XSS. Implementation: lightweight regex allowlist or `isomorphic-dompurify` if already available in the dependency tree.

### Prose styling

The container div receives Tailwind Typography classes (`prose prose-sm max-w-prose dark:prose-invert`) so that headings, lists, bold text, blockquotes, and code blocks are rendered with consistent, readable styles without any custom CSS.

**Dependency:** `@tailwindcss/typography` plugin added to `tailwind.config.ts`.
...
| `components/admin/product-form-sections.tsx` | Replace `<Textarea>` for `description` with dynamic import of `RichTextEditor` |
